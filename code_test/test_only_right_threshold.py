...
'''
@author: YYZhang
@file: test_only_right_threshold.py
@time: 5/20/20 5:34 PM
@purpose:
'''
#!/usr/bin/env python3
# encoding: utf-8
...
def report_dic2csv(report_dic,save_dir,probability_set,time_condi):
	save_dir = os.path.join(save_dir,'report')
	if not os.path.exists(save_dir):
		os.makedirs(save_dir)
	cls_dic = {2:'Ped',4:'Car'}
	#cls_dic = {2: 'Ped'}
	f = open(os.path.join(save_dir,'report_%s.csv'%(time_condi)),'a+')
	f.write(',TP,TN,FP,FN,ACC,PRE,RECALL,'+'probability_set:'+str(probability_set)+'\n')
	sum_dic = {2:{},4:{}}
	#sum_dic = {2: {}}
...
def predic_img(net,img,res_height,res_width,mask,probability_set):
...
def res_from_pc(probability_set):
	...
	save_dir = '/media/soterea/Data_ssd/work/YYZhang/test_file/experiment_results/2020/add_threshold'
	...
	save_dir_img = os.path.join(save_dir,'add_threhold_'+str(probability_set),'image')
	save_report = os.path.join(save_dir,'add_threhold_'+str(probability_set))
	...
	probability_txt_true_path = os.path.join(save_report,'probability_true.txt')
	probability_txt_false_path = os.path.join(save_report,'probability_false.txt')
	all_probability = []
	for frm_id in range(num_frm):
		...
		'''get pc res'''
		prediction,pred_probability = predic_img(net,img,res_height1,res_width,mask,probability_set)
	...
	report_dic2csv(report_dic_day, save_dir, probability_set,time_condi='day')
	report_dic2csv(report_dic_night, save_dir, probability_set,time_condi='night')
