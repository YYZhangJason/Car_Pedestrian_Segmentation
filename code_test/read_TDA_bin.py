...
'''
@author: YYZhang
@file: read_TDA_bin.py
@time: 3/30/20 7:18 PM
@purpose:
'''
# -*- coding:utf-8 -*-
# Analyze data
		...
		for cls_id in [2, 4]:
			if warning_gt[side][cls_id] == warning_tda[side][cls_id]:
				if warning_gt[side][cls_id] == 1:
					...
					report_dic[side][cls_id]['TN'] += 1
			else:
				if warning_gt[side][cls_id] == 1:
					report_dic[side][cls_id]['FN'] += 1
				else:
					report_dic[side][cls_id]['FP'] += 1
...
def resize_bsd_area(height, width):
	bsd_area = np.asarray(
		[[(230, 42), (66, 180), (868, 218), (764, 96)], [(234, 420), (32, 262), (918, 296), (758, 420)]], dtype=np.int)
...
def main():
	'''对TDA上检测的结果进行读取分析'''
	'''set param'''
	# tda_file = '/media/soterea/Data_ssd/work/01_Project/BSD_alarm/out/20191105_experiment/768x384_confirm/BGR_768x384_20191115.BIN'
	tda_file = '/media/soterea/Data_ssd/work/YYZhang/windows/test/2020/stats_tool_out.bin'  # result of TDA detect
	res_height1 = 384
	res_height2 = 192
	res_width = 768
	# record_file = '/media/soterea/Data_ssd/work/01_Project/TDA_HIL/data/resolution_ex/768x384_confirm/record.csv'
	record_file = '/media/soterea/Data_ssd/work/YYZhang/TDA_Detection_results/0/0record.csv'
	# save_dir = '/media/soterea/Data_ssd/work/01_Project/BSD_alarm/out/20191105_experiment/768x384_confirm'
	save_dir = '/media/soterea/Data_ssd/work/YYZhang/TDA_Detection_results/temp'
	# model = '/media/artur/Data_ssd/work/01_Project/BSD_weekly_training/experiment/resolution/train/800x400/sparse/deploy.prototxt'
	# weights = '/media/artur/Data_ssd/work/01_Project/BSD_weekly_training/experiment/resolution/train/800x400/sparse/BSD_Combi_d-n_191024_iter_30000.caffemodel'
	#
	...
	for frm_id in range(num_frm):
		...
		img_gt, warning_gt = draw_res_gt(img, label, bsd_area)
		...
		# print('warning_gt:',warning_gt)
		# print('warning_tda:',warning_tda)

		'''show'''
		if warning_gt != warning_tda:
			# if True:
			# 	print(warning_gt == warning_tda)
