...
def predic_img(net,img,res_height,res_width):
	...
	#mask = (prediction == 2) & (pred_ped < 0.8)
	#prediction[mask] = 0
...
def res_from_pc():
	...
	show_img = False
	save_res = True
	save_dir = '/media/soterea/Data_ssd/work/YYZhang/test_file/experiment_results/5.13_lingshiceshi/temp'
	...
	txt_path = '/media/soterea/Data_ssd/work/YYZhang/test_file/experiment_results/5.13_lingshiceshi/probability.txt'
	...
	for name in os.listdir(img_dir):
		...
		'''get pc res'''
		prediction ,pred_ped= predic_img(net,img_all,res_height1,res_width)
		...
		person_ = prediction[prediction==2].size
		if person_ ==0:
			continue
		pred_ped_sum = pred_ped[prediction==2]
		# pred_ped_sum = np.sum(pred_ped[prediction==2])

		#average_pred_probability = pred_ped_sum/person_#概率平均
		f = open(txt_path, 'a+')
		for i in range(len(pred_ped_sum)):

			f.write(str(pred_ped_sum[i]))
			f.write('\n')
			# f.write(name)
			# f.write('\n')
		f.close()
		#all_probability.append(pred_ped_sum)
		...
		plt.close()
	# average_all_img = np.sum(all_probability)/len(all_probability)
	# f = open(txt_path, 'a+')
	# f.write(str(average_all_img))
	# f.write('\t')
	# f.write('average_all_img')
	# f.write( '\n')
	# f.close()
